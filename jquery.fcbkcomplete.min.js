...
{option=$(option);if(option.attr('selected')||option.hasClass("selected"))
...
{$('li.pagination.noclick').remove();$.getJSON(options.json_url+"?tag="+etext,null,function(data)
...
{cache.push({caption:val.caption,value:val.value,paginate:val.paginate,extra:val.extra});if(val.caption!=''){search_string+=""+(cache.length-1)+":"+val.caption+";";}
if(val.paginate!=''){pagination=val.paginate;}});}
...
{content+='<li class="outer" rel="'+object.value+'">'+itemIllumination(object.caption,etext);content+=object.extra.content+'</li>';counter++;maximum--;}
match=myregexp.exec(search_string);}
if(pagination!=undefined)content+=pagination;feed.append(content);if(options.firstselected)
...
{feed.children("li").mouseover(function(e)
{feed.children("li").removeClass("auto-focus");$(this).addClass("auto-focus");focuson=$(this);if($(this).hasClass('outer')&&$(this).children("ul").css('display')!='block'){toggleSlide($(this).children("ul"));}});feed.children("li").mouseout(function(e)
{$(this).removeClass("auto-focus");focuson=null;});}
function toggleSlide(obj)
{$('.extra').slideUp('normal');obj.slideDown('normal');}
...
{var option=$(this);option.children('.extra').remove();addItem(option.text(),option.attr("rel"));feed.hide();browser_msie?browser_msie_frame.hide():'';complete.hide();});feed.children("li.noclick").unbind("mousedown");bindPaginateEvents();maininput.unbind("keydown");maininput.keydown(function(event)
{if(event.keyCode==191)
{event.preventDefault();return false;}
if(event.keyCode!=8)
{holder.children("li.bit-box.deleted").removeClass("deleted");}
if(event.keyCode==13&&checkFocusOn())
{var option=focuson;option.children('.extra').remove();addItem(option.text(),option.attr("rel"));complete.hide();event.preventDefault();focuson=null;return false;}
if(event.keyCode==13&&!checkFocusOn())
{if(options.newel)
{var value=xssPrevent($(this).val());value.children('.extra').remove();addItem(value,value);complete.hide();event.preventDefault();focuson=null;}
return false;}
if(event.keyCode==40)
...
{$("li.pagination.noclick a").click(function(e)
{var etext=xssPrevent($('.maininput').val());e.stopPropagation();e.preventDefault();$.getJSON($(this).attr('href'),null,function(data)
{addMembers(etext,data);json_cache=true;bindEvents();});return false;});}
...
var options=$.extend({json_url:null,cache:false,height:"10",newel:false,firstselected:false,filter_case:false,filter_hide:false,complete_text:"Start to type...",maxshownitems:30,onselect:"",onremove:""},opt);var holder=null;var feed=null;var complete=null;var counter=0;var pagination='';var extra=''
var cache=new Array();var json_cache=false;var search_string="";var focuson=null;var deleting=0;var browser_msie="\v"=="v";var browser_msie_frame;var element=$(this);var elemid=element.attr("id");init();return this;});};});